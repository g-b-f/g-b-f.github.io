 ...
 * returns A string description corresponding to the proficiency level.
 * @throws Will throw an error if the level is outside the range of 0 to 10.
 */
export function get_description(level: number){
    let level_int = Math.ceil(level)
    if (level_int > 10 || level_int < 0){
      throw new Error("get_description was passed level out of bounds")
    }
